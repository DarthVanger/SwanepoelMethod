(function() {
  ...
  var app = angular.module('swanepoelMethod', ['fileManagerModule', 'calculationEngineModule', 'plotterModule'])
    .config(function($interpolateProvider){
      $interpolateProvider.startSymbol('{[{').endSymbol('}]}');
    });

  //app.controller('cos', function($scope, $http, Calculus, Plotter, ExperimentalDataManager, FileSystemAPI) {
    //$scope.ExperimentalDataManager = ExperimentalDataManager;

    //$scope.experimentalData = 'not loaded yet';
    //$scope.$watch('ExperimentalDataManager.experimentalData', function(experimentalData) {
    ...
    //FileSystemAPI.saveFile('cosPoints.csv', ExperimentalDataManager.convertToCsv($scope.cosPoints)).then(function(response) {
